...
| AI 推理 | onnxruntime (ONNX) + ncnn (NCNN) | MobileCLIP2 模型推理 |
...
- `buildPhotoEmbeddingModelVersion(backend)` → `'mobileclip_image_ncnn_v1'` 等
...
- `mobileclip_backend_preference_service.dart` — 后端选择 (ONNX / NCNN)
- `ncnn_mobileclip_native_service.dart` — NCNN 原生推理

**模型资产:**
- ONNX: `assets/mobileclip2/s2/text_model.onnx`, `vision_model.onnx`
- NCNN: `assets/ncnn/mobileclip_s2/*.ncnn.param`, `*.ncnn.bin`
...
| MobileCLIP Vector Probe | Profile → 开发者设置 / Dart-Define | 对比 ONNX vs NCNN 向量 |
...
- AI 模型类型: MobileCLIP2 ONNX / NCNN (SegmentedButton)
