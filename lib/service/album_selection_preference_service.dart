...
class AlbumSelectionSnapshot {
  const AlbumSelectionSnapshot({required this.selectedAlbumIds});
...
class ScanFilterPreferences {
  ...
  bool get hasAnyRule =>
      minYear != null || hasMinResolution || minPixels != null;
...
class AlbumSelectionPreferenceService {
  ...
  Future<AlbumSelectionSnapshot> loadSelection() async {
    ...
    final selectedIds = prefs.getStringList(_selectedIdsKey) ?? defaultAlbumIds;
    return AlbumSelectionSnapshot(selectedAlbumIds: selectedIds);
  }

  Future<void> saveSelection({required List<String> selectedAlbumIds}) async {
    final prefs = await SharedPreferences.getInstance();
